...
class Piece:
    ...
    def __repr__(self) -> str:
        return f'Piece("{self.color}","{self.x_px}","{self.y_px}","{self.w}","{self.h}","{self.x}","{self.y}")'
...
def get_board_size(black_pieces: List[Piece], white_pieces: List[Piece]) -> Tuple[Tuple[int, int], Dict[float, int], Dict[float, int]]:  
    """
    ...
    x_positions = {round(statistics.median(vals), 0): grp for grp, vals in x_groups.items()}
    y_positions = {round(statistics.median(vals), 0): grp for grp, vals in y_groups.items()}
    ...
    # Divide largest px_pos by median distance to get number of intervals among dim. Add 1 to account for final piece's additional pxs.
...
def get_pieces(img: np.ndarray) -> Tuple[List[Piece], List[Piece]]:
    ...
    # Distance transform for a binary image will find the distance from a pixel to the closest non empty pixel
    ...
    _, thresh_blk = cv2.threshold(dist_transform, 0.7 * dist_transform.max(), 255, cv2.THRESH_BINARY)
    ...
    contours_blk = cv2.findContours(thresh_blk, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    contours_white = cv2.findContours(thresh_white, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)

    contours_blk, contours_white = imutils.grab_contours(contours_blk), imutils.grab_contours(contours_white)
